...
Messaging is **two-way but strictly directional**: control commands, output reports. Output emits four event types through `lib/outputAck.ts` — `OUTPUT_APPLIED` / `OUTPUT_CLEARED` / `OUTPUT_FAILED`, each carrying the originating `commandId` and the output page's per-load session id, and a low-frequency `OUTPUT_STATUS` heartbeat with the OBS Browser binding's source state (`null` without the binding — a plain tab claims nothing either way), which screen the page is, and whether OBS hosts it at all. **Presence is a map keyed by output session id** (`OutputStatusMap`), not one record: a rig runs several browser sources, and a single slot was rebuilt around whichever spoke last, so a dead second screen reported as healthy. Failures are recorded per output too — Program's own `outputFailure` deliberately never un-confirms, which with two sources swallowed the second screen's failure entirely. What an inbound message means for Program is one tested pure rule, `lib/programSync.ts`: **an acknowledgement confirms only the command whose id it carries** (a stale ack cannot confirm a newer Take), a published Clear stays `clearing` until the matching `OUTPUT_CLEARED`, remote commands are applied last-writer-wins by their own timestamp so every open control tells the same story, and confirmed claims decay to `UNVERIFIED` when the heartbeat goes stale (`lib/outputPresence.ts`). Even a confirmed, source-active reading is evidence about a page and a source — never "LIVE": nothing in the system observes an encoder or a stream.

**`OUTPUT READY` is the ceiling this dependency-free path can guarantee**, and it means "the output page received and applied the graphic" — established by the page's own acknowledgement, needing nothing from OBS. `OUTPUT ACTIVE` / `SOURCE HIDDEN` / `SOURCE INACTIVE` are **opportunistic**: they appear only when a real source-specific OBS reading arrives, and no reading means `OUTPUT READY`. Absence is never converted into active, hidden or inactive. On the rig tested for this — obs-browser 2.26.9 on macOS — the global `obsSceneChanged` arrived while `obsSourceActiveChanged`, `obsSourceVisibleChanged`, both legacy callbacks and even the page's own `visibilitychange` did not, so `OUTPUT READY` was the honest ceiling there; `docs/OBS_SETUP.md` records the full measurement. Where a build *does* report, the first report is the fragile one: `obsSourceActiveChanged` fires as the page is created, before any module has loaded, so `index.html` installs a parse-time listener that keeps it for the subscription to read. Without it an OBS restart with the scene already live sat at `OUTPUT READY` until someone switched scenes and back. Scene names are never turned into source truth: a global says which scene OBS switched to, not whether this source is in it.

**The working draft is local to one control client, and is not Program.** Three states stay deliberately separate: Program restores from its own storage and the relay snapshot; the editor's working draft (template, values, the current graphic's theme, layout, duration) restores from `lib/workingDraft.ts`; and the template library must not silently replace a restored draft on mount, though an explicit template choice still starts a new graphic. Neither state hydrates the other — Program never becomes the draft, and restoring a draft writes nothing to Program, the output presence map or the pending acks. It lives in **sessionStorage**, because a Chrome studio and an OBS dock share one Program truth while legitimately preparing different graphics, and two `/control` tabs in one browser must not overwrite each other as a single localStorage record would; the draft therefore never reaches a realtime message, the relay, BroadcastChannel or `OUTPUT_STATUS`. Persistence is one debounced subscription rather than a save in each setter, validation is all-or-nothing (absent, corrupt, wrong version, unknown template or any invalid field falls back to the pack seed rather than restoring half a record), and "Reset all local data" removes it. Which draft fields are asset sources is decided **by key, never by reading the value** — `*AssetId` slots persist as stable ids (the same `endsWith` convention `lib/rundown/rundownReferences.ts` uses), `logoUrl` persists unless it carries inline binary or a dead object URL, `/output`'s render-only `logoResolvedSrc`/`headshotResolvedSrc` never persist, and every other value round-trips byte for byte — so an announcement that begins "Data: registration closes at 5 PM" is ordinary text, not an asset. Two things were measured rather than assumed: a real page refresh preserves the draft, and hiding then reshowing an OBS Custom Browser Dock from `View > Docks` does not destroy that dock's storage context. OBS's own title-bar Refresh was unavailable on the tested build and is not claimed either way. A genuinely destroyed and recreated context — tab closed, OBS restarted — starts fresh and the editor seeds normally.
...
`/control` is a layout route with five workspaces inside it (`/control/studio`, `/control/scripture`, `/control/rundown`, `/control/library/:section`, `/control/screens`) and two responsive layouts, switched by a JS breakpoint at 1024px — the layouts are responsive, not separate routes: a guided **dock** below 1024px and a multi-panel **studio** dashboard above it. **One command owner:** the layout route itself owns the realtime channel, the publish path, the in-flight guard and both Take handlers, so a workspace can request a graphic but no workspace can air one. Workspaces receive only what they need through the outlet context. State lives in a **Zustand** store; presets, brand overrides, and recents persist to `localStorage` and are validated with **Zod** on read. The control preview and `/output` share the same renderer, so the preview is pixel-true to air. See `CONTROL_UI_UX.md`.

## Output screens

A rig runs several output sources, and each declares **which screen it is** in
its own URL (`/output?screen=split`) — a browser source's identity is its
address and nothing else can carry it. Screens are data (`lib/scriptureOutputs.ts`):
`main`, `lower`, `split`, `house`, each mapped to a look.

The mapping re-skins **`scripture-card` and nothing else**, gated once in
`OutputPage.tsx` through the shared `resolveScreenValues`, which the Screens
page's previews call too so the two cannot drift. It is applied to the values
handed to the renderer and never written back, so `OUTPUT_APPLIED` still
acknowledges the command as sent and Recent, presets and the rundown keep the
look the operator chose. An unknown or absent `screen` is the main screen, which
is what keeps every existing browser-source URL working.

The mapping travels as a retained realtime message, not just a local setting:
the control surface is usually a different *browser* from the output (Chrome and
OBS CEF share no localStorage), so a setting that only persisted locally would
never reach the screen it configures.
